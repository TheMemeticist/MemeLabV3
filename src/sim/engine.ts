...
export class Engine {
  ...
  step(): SimStats {
    const pop = this.pop;
    const { state, next, age, infectedAge, defenses, strainId, size, n } = pop;
    ...
    for (let i = 0; i < n; i++) {
      ...
      const srcMul = sourceControlMultiplier(D, defenses[i]);
      ...
      for (let k = 0; k < m2; k += 2) {
        ...
        const protMul = protectionMultiplier(D, defenses[j]);
  ...
  buffers(): { state: Uint8Array; defenses: Uint8Array; size: number } {
    return { state: this.pop.state, defenses: this.pop.defenses, size: this.pop.size };
  }
}
